...
class AuthServiceTest {
	...
	void signupSuccess() {
		...
		given(memberService.existsByKakaoId(kakaoId)).willReturn(false);
		given(memberService.existsByNickname(nickname)).willReturn(false);
		given(memberService.save(any(Member.class))).willReturn(member);
		given(memberTagService.findAllByIds(tagIds)).willReturn(List.of(new MemberTag(), new MemberTag()));
		given(memberTagMappingRepository.save(any(MemberTagMapping.class))).willReturn(null); // 추가된 부분
	...
	void signupFailIfAlreadyExists() {
		...
		given(jwtTokenProvider.isValidToken(signupToken, JwtTokenType.SIGNUP)).willReturn(true);
		given(jwtTokenProvider.extractKakaoId(signupToken)).willReturn(kakaoId);
		given(memberService.existsByKakaoId(kakaoId)).willReturn(true);
